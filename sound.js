...
function Sound(filename, basePath, onError, onTimeUpdate, options) {
  ...
  this.registerTimeUpdateHandler = function () {
    if (this.onTimeUpdateSubscription != null) {
      console.warn('On time update event listener is already registered');
      return;
    }
    this.onTimeUpdateSubscription = eventEmitter.addListener(
      'timeUpdate',
      (param) => {
        const { currentTime, isPlaying, playerKey } = param;
  ...
  RNSound.prepare(this._filename, this._key, options || {}, (error, props) => {
    ...
    if (error === null) {
      this._loaded = true;
      this.registerOnPlay();
      this.registerTimeUpdateHandler();
    }
